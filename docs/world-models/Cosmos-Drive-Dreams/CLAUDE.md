...
| `assets/` | `assets/example/` — a single complete RDS-HQ clip, enough to run the pipeline with no download |
| `environment.yaml`, `requirements.txt`, `INSTALL.md` | conda-based setup (see gotchas — requirements.txt is partly broken) |
...
# submodule URL is SSH upstream; use HTTPS explicitly:
...
pip install -r requirements.txt              # see gotchas first
...
Docker: the top-level repo has **no** Dockerfile. Use the one in this directory
(`Dockerfile`), which covers the toolkit/rendering path. `cosmos-transfer-lidargen/Dockerfile`
exists separately for the LiDAR subtree (`nvcr.io/nvidia/pytorch:24.10-py3` + miniconda).
...
# 3. front-view generation (needs the submodule + ~300 GB of checkpoints)
...
# 4. single-to-multiview expansion
...
python cosmos-drive-dreams-toolkits/convert_waymo_to_rds_hq.py --help
python cosmos-drive-dreams-toolkits/rectify_ftheta_to_pinhole.py -i VIDEO -o OUT -r RDS_HQ_FOLDER -c CLIP_ID
```
...
About **300 GB**. You must first accept the terms for `meta-llama/Llama-Guard-3-8B`,
`nvidia/Cosmos-Tokenize1-CV8x8x8-720p` and `nvidia/Cosmos-Guardrail1`, plus the gated
`nvidia/Cosmos-Transfer1-7B-Sample-AV`. Checkpoints are `.pt` (pickle), not safetensors.
Env vars: `HF_TOKEN`, `HF_HOME`, `CUDA_HOME`, `PYTHONPATH=cosmos-transfer1`,
...
- Two `.gitmodules` are tracked; one filename starts with an invisible U+200E character.
  ...
  Delete the duplicate and set the HTTPS URL.
- `cosmos-transfer-lidargen/README.md` documents an optional `pip install ncore
  --extra-index-url https://__token__:${YOUR_TOKEN}@gitlab-master.nvidia.com/...`. It is
  NVIDIA-internal; do not run it (it leaks tokens into history and logs).
- `cosmos-drive-dreams-toolkits/utils/animation_utils.py:680` calls
  `eval(f"self.scene.{attrib_name} = attrib_val")`. Dead-but-reachable code; remove it.
- Rendering parallelism is Ray. Set `USE_RAY=False` inside `render_from_rds_hq.py` to debug
  in a single process.
- LiDAR and world-scenario rendering need a GPU plus moderngl — they fail on headless boxes
  without an EGL-capable driver.
...
- Data format: **RDS-HQ** — a directory-of-folders layout (`pose`, `vehicle_pose`,
  `all_object_info`, `captions`, `ftheta_intrinsic`, `pinhole_intrinsic`, `3d_*`, `lidar_raw`).
  This is the interchange format; convert into it rather than changing the code.
