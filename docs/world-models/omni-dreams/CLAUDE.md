...
live in https://github.com/NVIDIA/flashdreams.
...
| Path | What |
|---|---|
| `samples/post-training/` | User-facing sample: `QUICKSTART.md`, `setup_env.sh`, `_env.sh`, `prepare.py` (dataset staging), `torchrun_smoke.sh`, `smoke_test.slurm`, `configs/`, `tests/` |
| `post-training/` | The package: `omnidreams/` (public layer + vendored `_src/`), `packages/cosmos-oss/`, `Dockerfile`, `justfile`, `uv.lock`, `COMMIT.txt` |
| `post-training/omnidreams/experiments/` | Release config bases: `causal/`, `causal_multiview/`, `self_forcing/` |
| `post-training/packages/cosmos-oss/vqa/` | VQA evaluator (Cosmos-Reason scores generations) |
| `skills/run-post-training-sample/` | Agent skill: run E1/E2/E3 on 8-256 H100s |
| `.github/workflows/` | `lint.yml`, `post-training.yml` (self-hosted runners) |
| `AGENTS.md`, `REUSE.toml`, `NOTICE`, `THIRD_PARTY_NOTICES.txt` | Agent routing, licence compliance |
...
```bash
# caches MUST be set before uv sync (flash-attn builds are large); use a >=150 GB filesystem
export OMNI_CACHE_DIR=$HOME/.cache/omni-dreams
export UV_CACHE_DIR=$OMNI_CACHE_DIR/uv TMPDIR=$OMNI_CACHE_DIR/tmp
...
# export OMNI_DREAMS_HF_ORG=<org>          # only if ckpts live under a non-nvidia org
...
`OMNI_HF_DATA_{REVISION,SUBPATH,INCLUDE}`, and `OMNI_LOCAL_DATA_SOURCE` (local mirror).
...
sbatch samples/post-training/smoke_test.slurm    # edit #SBATCH --account/--partition first
```
...
uv run pytest samples/post-training/tests/     # setup_env, prepare, torchrun, cuda stack
```
CI: `.github/workflows/lint.yml` + `post-training.yml`, on `[self-hosted, omni-dreams]`.

## Weights

All gated; accept the licences with the **same account** whose token you stage:
- `nvidia/omni-dreams-models` — Cosmos-Dreams checkpoints (NVIDIA Open Model License). May
  be restricted to an authorized org; set `OMNI_DREAMS_HF_ORG` if so.
...
- `nvidia/PhysicalAI-Autonomous-Vehicles-NuRec` branch `26.01` — sample dataset, under a
  **separate** NuRec Dataset License Agreement (the Open Model License does not cover it).

Verify with `hf auth whoami` plus a non-downloading `HfApi().list_repo_files(...)`.

## Gotchas found during review

- **This repo cannot generate video.** Inference is in NVIDIA/flashdreams; do not clone
  FlashDreams for post-training.
- `HF_TOKEN` is **ignored** — the token must be in `$OMNI_CACHE_DIR/huggingface/token`.
- `NPROC < 8` is unsupported; do not run the release configs on fewer GPUs.
...
- Do not run `torchrun_smoke.sh` on a Slurm login node; it fails with a CUDA error.
- `post-training/omnidreams/_src/` is a vendored export of an internal NVIDIA GitLab branch
  (`post-training/COMMIT.txt`), and `post-training/INTERNAL.md` is referenced by configs but
  absent. Treat `_src` as an unauditable blob; avoid editing it.
- `_src/imaginaire/datasets/webdataset/decoders/pickle.py:18` calls `pickle.loads` on shard
  members — never train on shards we did not build. LazyConfig `.py` files are `exec`'d
  (`_src/imaginaire/lazy_config/lazy.py:153,215`).
- CI uses self-hosted runners with plain `pull_request` triggers; the workflows themselves
  flag this as unsafe for a public repo. Fix before mirroring internally.
...
- Licence: `REUSE.toml` + SPDX headers; `just license`, `just release-check`. DCO sign-off
  required on commits (`CONTRIBUTING.md`).
